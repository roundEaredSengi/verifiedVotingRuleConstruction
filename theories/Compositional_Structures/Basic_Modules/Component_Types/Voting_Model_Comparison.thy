...
locale svg_rule_power_comparison = 
  equiv: 
    power_equivalence \<V> \<B> "UNIV::bool set" \<O> "UNIV::bool set" \<M> \<M>' \<delta> \<delta>' mech mech'
  for 
...
interpretation trivial_comparison: svg_rule_power_comparison 
  "{}" "{}" "{default}" "{}" "{}" "\<lambda>r v. 0" "\<lambda>g v. 0" trivial_rule_mech trivial_mech\<^sub>S\<^sub>V\<^sub>G
...
proof (simp, safe)
  ...
  have "\<forall>(m', f')\<in>\<M>'. \<exists>m f. (m, f) \<in> \<M> \<and> 
    model_isomorphism \<V> \<B> UNIV \<O> UNIV m m' f f' mech mech'"
    using equiv.moiso.correspondence\<^sub>l 
    by blast
  with valid_game have
    "\<exists> m f. (m, f) \<in> \<M> \<and> model_isomorphism \<V> \<B> UNIV \<O> UNIV m (V, vf) f f' mech mech'"
    by blast
  then obtain m :: "('v \<Rightarrow> 'b) \<Rightarrow> 'o" and f :: "('v \<Rightarrow> 'b) \<Rightarrow> 'o"  where
    valid_rule: "(m, f) \<in> \<M>" and 
    iso: "model_isomorphism \<V> \<B> UNIV \<O> UNIV m (V, vf) f f' mech mech'"
    by blast
...
next
  ...
  have "\<forall>(r, f)\<in>\<M>. \<exists>g f'. (g, f') \<in> \<M>' \<and> 
    model_isomorphism \<V> \<B> UNIV \<O> UNIV r g f f' mech mech'"
  ...
  with valid_rule obtain g :: "'v Simple_Voting_Game" and f' :: "('v \<Rightarrow> bool) \<Rightarrow> bool" where
    ...
    iso: "model_isomorphism \<V> \<B> UNIV \<O> UNIV r g f f' mech mech'"
    by blast
